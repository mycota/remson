  ...
  // drop dwon functions 
    ...
    $totalTowerOrd = $_POST["totalTowerOrd"];
        ...
        $totalTowerOrd_clean = mysqli_real_escape_string($con,$totalTowerOrd[$count]);
  			...
  			$insert_query = mysqli_prepare($con,"INSERT INTO tbl_production (proDate, CustID, jobNumber, line, type, hr, cutting, coating, assemb, pack, disp, glassOrdered, orderDate, deliDate, fabricate,final_date_deli, total_tower, total_tower_ordered, remarks) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        logs($_SESSION['id'], $_SESSION['username'], 'Added entry'.' for '.' customer ');
  ...
  <div class="row">
    ...
    <div class="container">
      ...
      <center><h1 class="page-header" style="background-color: #00CED1; color: white;">
        Add rows to begin entries</h1></center>
...
$(document).ready(function(){
 ...
 $(document).on('click', '.add', function(){
  ...
  html += '<td><input name="fab[]" type="text" class="form-control fab" placeholder="Enter fab ordered"/></td>';
 ...
 $('#insert_form').on('submit', function(event){
  ...
  $('.jobNumber').each(function(){
   ...
   if($(this).val() == '')
   {
    error += "<p>Enter job number at "+count+" Row</p>";
    return false;
   }
   count = count + 1;
  });
  ...
  //  if($(this).val() == '')
  //  {
  //   error += "<p>Enter fab at "+count+" Row</p>";
  //   return false;
  //  }
  //  count = count + 1;
  // });
